...
public class StandardLibrary
{
    ...
    private readonly Dictionary<string, double> floatConstants = new Dictionary<string, double>();
    ...
    public string GetFunctionDefinitions()
    {
        ...
        if (UsedSymbols.Count > 0 || floatConstants.Count > 0)
        {
            sb.AppendLine(".data");
            foreach (var sym in UsedSymbols)
                sb.AppendLine(Symbols[sym]);
            foreach (var kv in floatConstants)
                sb.AppendLine($"{kv.Key}: .double {kv.Value.ToString(CultureInfo.InvariantCulture)}");
        }

        sb.AppendLine(".text");
        foreach (var fn in UsedFunctions)
            sb.AppendLine(FunctionDefinitions[fn]);
...
print_done:
    ...
    "},
    { "strcmp", @"<pegas aquí el código ensamblador arriba>" },
